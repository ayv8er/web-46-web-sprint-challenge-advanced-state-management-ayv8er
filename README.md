...
1. What problem does the context API help solve?
   It resolves not needing to prop drill state through components that might not need it, and directly injecting it to the component that needs it.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
   The store is the location and method of accessing global state. Actions are the objects explaining the function action which is created and passed along to the reducer. The reducer accepts an action and runs it through a list of possible actions.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
   redux-thunk allows us to perform asynchronous calls before being passed to the reducer. It allows us to perform a dispatch inside of another dispatch when a promise is returned.

4. What is your favorite state management system you've learned and this sprint? Please explain why!
   I don't know if I have a favorite yet. I am comfortable with Redux and I'm glad I am as it is the industry standard. I guess my favorite is what I'm comfortable with. But I do think I could fall in love with Context API due to the less overhead required in setting up. But I think it would be easy to get lost in Context API because with prop drilling there was a crumb trail of where each prop came from, or in the case with Redux, you know which props are available to you on each Component through mapStateToProps.
